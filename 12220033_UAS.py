...
negara_1 = df_info["name"].tlist()

negara = st.selectbox("Pilih negara : ",negara_1) 

code = df_info[df_info["name"]==negara]["alpha-3"].tlist()[0]

st.write("Kode negara : ",code)
st.write("Negara : ",negara)

x_1 = df_[df_["kode_negara"]==code]["tahun"].tlist()
y_1 = df_[df_["kode_negara"]==code]["produksi"].tlist()
...
df = df[df["tahun"]==T]
code_negara = df[df["tahun"]==T]["kode_negara"].tlist()
# produksi = df[df['tahun']==T]['produksi'].tlist()
...
for code in code_negara:
    try:
        produksi = df[df["kode_negara"]==code]["produksi"].tlist()
        negara = dfJ[dfJ["alpha-3"]==code]["name"].tlist()[0]
...
B_1 = st.sidebar.number_input("Berapa besar negara (Bagian C)?", min_value=1, max_value=None)

df = df_
dfJ = df_info

code_negara = df["kode_negara"].tlist()
code_negara = list(dict.fromkeys(code_negara))
...
year = list(dict.fromkeys(df["tahun"].tlist()))
...
for t in year:
    ...
    produksi = np.array(df_per_year["produksi"].tlist())
    max_produksi = max(produksi)
    min_produksi = min([p for p in produksi if p != 0])
    nol_produksi = min([p for p in produksi if p == 0])
    # maksimum
    code_negara = df_per_year[df_per_year["produksi"]==max_produksi]["kode_negara"].tlist()[0]
    if code_negara == "WLD":
        code_negara = "WLF"
    dic_max["negara"].append(dfJ[dfJ["alpha-3"]==code_negara]["name"].tlist()[0])
    dic_max["kode_negara"].append(code_negara)
    dic_max["region"].append(dfJ[dfJ["alpha-3"]==code_negara]["region"].tlist()[0])
    dic_max["sub_region"].append(dfJ[dfJ["alpha-3"]==code_negara]["sub-region"].tlist()[0])
    dic_max["produksi"].append(max_produksi)
    # minimum != 0
    code_negara = df_per_year[df_per_year["produksi"]==min_produksi]["kode_negara"].tlist()[0]
    if code_negara == "WLD":
        code_negara = "WLF"
    dic_min["negara"].append(dfJ[dfJ["alpha-3"]==code_negara]["name"].tlist()[0])
    dic_min["kode_negara"].append(code_negara)
    dic_min["region"].append(dfJ[dfJ["alpha-3"]==code_negara]["region"].tlist()[0])
    dic_min["sub_region"].append(dfJ[dfJ["alpha-3"]==code_negara]["sub-region"].tlist()[0])
    dic_min["produksi"].append(min_produksi)
    # zero == 0
    code_negara = df_per_year[df_per_year["produksi"]==nol_produksi]["kode_negara"].tlist()[0]
    if code_negara == "WLD":
        code_negara = "WLF"
    dic_zero["negara"].append(dfJ[dfJ["alpha-3"]==code_negara]["name"].tlist()[0])
    dic_zero["kode_negara"].append(code_negara)
    dic_zero["region"].append(dfJ[dfJ["alpha-3"]==code_negara]["region"].tlist()[0])
    dic_zero["sub_region"].append(dfJ[dfJ["alpha-3"]==code_negara]["sub-region"].tlist()[0])
